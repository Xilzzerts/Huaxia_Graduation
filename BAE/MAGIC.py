...
class Content:
    ...
    def check_password(self):
        ...
        recv = open_handler.read().decode('GBK')
        ...
        if recv.find("系统警告") != -1:
            ...
            "get VIEWSTATE"
            soup = BeautifulSoup(recv, from_encoding='GBK')
            ...
            res = recv_index.read().decode('GBK')
    ...
    def get_stuinfo(self):
        ...
        si = BeautifulSoup(info_s, from_encoding='GBK')
        soup = BeautifulSoup(si.prettify(), from_encoding='GBK')
    ...
    def get_timetable(self):
        try:
            timetable_info = self.opener.open(self.addr + TIMETABLE_PAGE + "?xh=" + self.num + "&type=xs",
                                             timeout=TIMEOUT)
        except URLError as e:
            if str(e.reason) == "timed out":
                self.timeout = True
        ...
        timetable_info_s = timetable_info.read().decode("GBK")
        if timetable_info_s.find("系统警告") != -1:
            self.system_error = True
            self.opener.close()
            return

        soup = BeautifulSoup(timetable_info_s, from_encoding='GBK')
    ...
    def get_transcript(self):
        ...
        soup = BeautifulSoup(grade_info_s, from_encoding='GBK');
...
class XMLGenerator:
    ...
    def add_status(self):
        ...
        temp.setAttribute("timetable", str(self.content.timetable_ok))
        temp.setAttribute("transcript", str(self.content.transcript_ok))
        temp.setAttribute("timeout", str(self.content.timeout))
        self.node.appendChild(temp)
...
class MainHandler(tornado.web.RequestHandler):
	def get(self):
		...
		logging.debug(num + " " + pwd + " " + server)
		if num:
			c = Content(num, pwd, int(server))
			logging.debug(c.stuinfo)
			x = XMLGenerator(c)
			outbyte = x.mainnode.toprettyxml(encoding="UTF-8")
			out = str(outbyte)
			self.set_header("Content-Type", "application/xml; charset=utf-8")
			self.set_header("Server", "Zirconi's")
			self.write(out)
		else:
			self.write("Anything is possible")
			
class IndexHandler(tornado.web.RequestHandler):
    def get(self):
        self.write("Zirconi's FINAL")
			

app = tornado.wsgi.WSGIApplication([(r"/", IndexHandler), (r"/MAGIC", MainHandler),])
application = WSGIApplication(app)

#if __name__ == "__main__":
#    application = tornado.web.Application([(r"/MAGIC", MainHandler),])
